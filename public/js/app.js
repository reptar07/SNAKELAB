...
const App = {
  ...
  setupScrollWorld() {
    ...
    // Easing function para movimientos suaves
    const easeInOutCubic = (t) => t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2;
    
    // Función para calcular posición relativa suave
    const calculateSceneEffect = (card) => {
      const rect = card.getBoundingClientRect();
      const viewportCenter = window.innerHeight / 2;
      const cardCenter = rect.top + rect.height / 2;
      
      // Distancia desde el centro (-1 a 1)
      const rawDistance = (cardCenter - viewportCenter) / (window.innerHeight / 2);
      const distance = Math.max(-1, Math.min(1, rawDistance));
      
      // Aplicar easing
      const easedDistance = easeInOutCubic(Math.abs(distance));
      const signedEasedDistance = distance < 0 ? -easedDistance : easedDistance;
      
      return {
        distance: signedEasedDistance,
        rawDistance: distance,
        progress: Math.abs(distance),
        inView: Math.abs(distance) < 0.5
      };
    };

    // Función para aplicar transformaciones
    const updateCardVisuals = (card, effect) => {
      const img = card.querySelector('.scene-image img');
      if (!img) return;

      // Calcular valores
      const scale = 1 + (1 - effect.progress) * (config.maxScale - 1);
      const yShift = effect.rawDistance * config.maxYShift;
      const opacity = Math.max(0.4, 1 - effect.progress * 0.6);
      const brightness = config.minBrightness + (1 - effect.progress) * (config.maxBrightness - config.minBrightness);
      const saturate = 0.8 + effect.progress * 0.2;

      // Aplicar transformación a la imagen (sin rotación)
      img.style.transform = `scale(${scale}) translateY(${yShift}px)`;
      img.style.opacity = opacity;
      img.style.filter = `brightness(${brightness}) saturate(${saturate})`;

      // Aplicar transformación a la tarjeta
      card.style.opacity = opacity;
      card.style.transform = `translateY(${-effect.rawDistance * 15}px) scale(${0.95 + effect.progress * 0.05})`;

      // Añadir clase active si está centrado
      if (effect.inView) {
    ...
    const updateScrollEffects = () => {
      sceneCards.forEach(card => {
        const effect = calculateSceneEffect(card);
        updateCardVisuals(card, effect);
      });
    };

    // Optimized scroll listener con RAF
    const handleScroll = () => {
      if (animationFrameId) {
