...
const ProductPage = {
  ...
  render() {
    ...
    const sizePrices = this.parseSizePrices(product.size_prices || '{}');
    const variants = this.parseJSON(product.variants, []);

    this.renderExtendedProductInfo(product, { images, sizes, colors, materials });
    this.renderProductPrice(product, sizes, sizePrices);
    this.renderProductImageFallback(product, images);
    ...
    if (detailsEl) {
      detailsEl.innerHTML = `
        ...
        <p class="modal-description" id="modal-description">${Security.escapeHtml(product.description)}</p>
        
        <div class="modal-price-row">
          <span id="modal-price" class="modal-price">${Cart.formatPrice(product.price)}</span>
          <span id="modal-compare-price" class="modal-compare-price" ${product.compare_price ? '' : 'style="display:none;"'}>${product.compare_price ? Cart.formatPrice(product.compare_price) : ''}</span>
          <span id="modal-discount-badge" class="modal-discount-badge" ${discountHTML ? '' : 'style="display:none;"'}>${discountHTML || ''}</span>
        </div>
        ...
        ${sizes.length > 0 ? `
          <div class="option-group">
            ...
            <div class="option-chips" id="size-chips">
              ${sizes.map((s, i) => `<button class="option-chip ${i === 0 ? 'selected' : ''}" data-value="${Security.escapeHtml(s)}" onclick="ProductPage.selectOption(this, 'size-chips'); ProductPage.updateSelectedSizePrice('${Security.escapeHtml(s)}')">${Security.escapeHtml(s)}</button>`).join('')}
            </div>
          </div>
        ` : ''}
        ...
        ${variants.length > 0 ? `
          <div class="option-group">
            <span class="option-label">Referencia</span>
            <div class="option-chips variant-chips" id="variant-chips">
              ${variants.map((variant, index) => `<button type="button" class="option-chip ${index === 0 ? 'selected' : ''}" data-variant-index="${index}" onclick="ProductPage.selectVariant(${index})">${Security.escapeHtml(variant.name)}</button>`).join('')}
            </div>
          </div>
        ` : ''}
    ...
    if (variants.length) this.applyVariant(variants[0], 0);

    this.setup3D(product, colors);
  },

  applyVariant(variant, index = null) {
    if (!variant) return;
    this.selectedVariant = variant;
    const price = Number(variant.price) || Number(this.currentProduct.price) || 0;
    const comparePrice = Number(variant.comparePrice) || 0;
    const descriptionEl = document.getElementById('modal-description');
    const priceEl = document.getElementById('modal-price');
    const compareEl = document.getElementById('modal-compare-price');
    if (descriptionEl && variant.description) descriptionEl.textContent = variant.description;
    if (priceEl) priceEl.textContent = Cart.formatPrice(price);
    if (compareEl) {
      compareEl.textContent = comparePrice > price ? Cart.formatPrice(comparePrice) : '';
      compareEl.style.display = comparePrice > price ? 'inline' : 'none';
    }
    const imageUrl = Security.imageUrl(variant.image);
    if (imageUrl) {
      const mainImageEl = document.getElementById('modal-main-image');
      if (mainImageEl) mainImageEl.innerHTML = `<img src="${Security.escapeHtml(imageUrl)}" alt="${Security.escapeHtml(this.currentProduct.name)}" style="animation: fadeIn 0.3s ease">`;
    }
    document.querySelectorAll('#variant-chips .option-chip').forEach((chip, chipIndex) => chip.classList.toggle('selected', chipIndex === index));
  },

  selectVariant(index) {
    const variants = this.parseJSON(this.currentProduct?.variants, []);
    this.applyVariant(variants[index], index);
  },

  setup3D(product, colors) {
    const configurator = document.querySelector('.maker-configurator');
    const show3D = product.show_3d !== 0 && product.show_3d !== '0'
      && product.show_3d !== false && product.show_3d !== 'false';
    if (configurator && !show3D) {
      configurator.hidden = true;
      return;
    }
    const mount = () => window.Product3D?.mount(product, colors);
    if (window.Product3D) {
      mount();
    } else {
      window.addEventListener('snake-lab-3d-ready', mount, { once: true });
    }
  },

  selectColorFrom3D(hex) {
    this.selected3DColor = hex;
    const colorChip = Array.from(document.querySelectorAll('#color-chips .color-chip'))
      .find(chip => String(chip.dataset.value).toLowerCase() === String(hex).toLowerCase());
    if (colorChip) this.selectOption(colorChip, 'color-chips');
  },

  get3DColorSelections() {
    return window.Product3D?.getSelections?.() || {};
  },
  ...
  parseSizePrices(value) {
    try {
      ...
      return Object.entries(parsed).reduce((acc, [size, price]) => {
        ...
        if (normalizedSize && Number.isFinite(numeric)) acc[normalizedSize] = numeric;
        return acc;
      }, {});
    } catch (e) {
      return {};
  ...
  resolveSizePrice(product, size, fallbackPrice = Number(product.price) || 0) {
    const sizePrices = this.parseSizePrices(product.size_prices || '{}');
    const selected = String(size || '').trim();
    if (!selected) return fallbackPrice;
    return Number(sizePrices[selected]) || fallbackPrice;
  },

  renderProductPrice(product, sizes, sizePrices) {
    const priceEl = document.getElementById('modal-price');
    const compareEl = document.getElementById('modal-compare-price');
    const discountEl = document.getElementById('modal-discount-badge');
    const defaultSize = sizes[0] || null;
    const selectedSize = document.querySelector('#size-chips .option-chip.selected')?.dataset.value || defaultSize;
    const currentPrice = this.resolveSizePrice(product, selectedSize, Number(product.price) || 0);
    const currentCompare = product.compare_price ? this.resolveSizePrice(product, selectedSize, Number(product.compare_price) || 0) : null;

    if (priceEl) priceEl.textContent = Cart.formatPrice(currentPrice);
    if (compareEl) {
      if (currentCompare && Number(currentCompare) > Number(currentPrice)) {
        compareEl.textContent = Cart.formatPrice(currentCompare);
    ...
    if (discountEl) {
      const discountValue = currentCompare && Number(currentCompare) > Number(currentPrice)
        ? Math.round((1 - Number(currentPrice) / Number(currentCompare)) * 100)
        : 0;
      if (discountValue > 0) {
    ...
    if (product && currentCompare && typeof product.compare_price === 'number') {
      product.compare_price = currentCompare;
    }
  },

  renderProductImageFallback(product, images) {
    const mainImageEl = document.getElementById('modal-main-image');
    if (!mainImageEl) return;
    const activeImage = Security.imageUrl(images[0]);
    if (activeImage) {
      mainImageEl.innerHTML = `<img src="${Security.escapeHtml(activeImage)}" alt="${Security.escapeHtml(product.name)}" style="animation: fadeIn 0.3s ease">`;
    }
  },
  ...
  renderSTLPreview(modelUrl) {
    ...
    const isUnsupported3D = /\.(stl|obj)$/i.test(modelUrl || '');
    if (isUnsupported3D) {
      host.innerHTML = `
        <div class="maker-viewport-overlay"></div>
        <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(10,13,19,0.25);color:var(--text-muted);font-size:0.9rem;letter-spacing:0.04em;">Vista 3D no disponible para este archivo</div>
      `;
      return;
    }
  ...
  render3DPreview(product, colors) {
    ...
    const supported3D = modelUrl && /\.(glb|gltf)(\?.*)?$/i.test(modelUrl);

    if (!supported3D) {
      if (host) {
        const staticImage = Security.imageUrl(this.parseJSON(product.images, [])[0]);
        host.innerHTML = `
          <div class="maker-viewport-overlay"></div>
          <img src="${Security.escapeHtml(staticImage || '/uploads/images/placeholder.jpg')}" alt="${Security.escapeHtml(product.name || 'Producto')}" style="width:100%;height:100%;object-fit:cover;display:block;position:relative;z-index:2;" onerror="this.src='/uploads/images/placeholder.jpg'">
        `;
      }
      viewer.style.display = 'none';
      if (infoEl) {
        infoEl.textContent = 'La vista 3D no está disponible para este producto. Se muestra la vista estática del diseño.';
      }
      return;
    }
  ...
  updateSelectedSizePrice(selectedSize = null) {
    const product = this.currentProduct;
    if (!product) return;
    const sizeChips = document.querySelectorAll('#size-chips .option-chip');
    const activeSize = selectedSize || document.querySelector('#size-chips .option-chip.selected')?.dataset.value;
    const chosenSize = activeSize || this.parseJSON(product.sizes, [])[0] || null;
    const currentPrice = this.resolveSizePrice(product, chosenSize, Number(product.price) || 0);
    const compareBase = Number(product.compare_price) || 0;
    const currentCompare = chosenSize && compareBase ? this.resolveSizePrice({ ...product, compare_price: compareBase }, chosenSize, compareBase) : compareBase;
    ...
    if (priceEl) priceEl.textContent = Cart.formatPrice(currentPrice);
    if (compareEl) {
      if (currentCompare && currentCompare > currentPrice) {
        compareEl.textContent = Cart.formatPrice(currentCompare);
        compareEl.style.display = 'inline';
      } else {
        compareEl.textContent = '';
    ...
    if (discountEl) {
      const discountValue = currentCompare && currentCompare > currentPrice
    ...
    product.price = currentPrice;
    if (currentCompare > 0) product.compare_price = currentCompare;

    sizeChips.forEach((chip) => {
      chip.classList.toggle('selected', chip.dataset.value === chosenSize);
    });
  },
  ...
  selectOption(el, groupId) {
    ...
    if (groupId === 'size-chips') {
      this.updateSelectedSizePrice(el.dataset.value);
    }
  },
  ...
  getSelectedOptions() {
    ...
    const selectedSize = getSelected('size-chips');
    const selectedColor = this.selected3DColor || getSelected('color-chips');
    const basePrice = Number(this.currentProduct?.price) || 0;
    const chosenPrice = this.resolveSizePrice(this.currentProduct, selectedSize, basePrice);

    return {
      size: selectedSize,
      color: selectedColor,
      colorHex: selectedColor,
      colorSelections: this.get3DColorSelections(),
      variant: this.selectedVariant?.name || null,
      variantImage: this.selectedVariant?.image || null,
